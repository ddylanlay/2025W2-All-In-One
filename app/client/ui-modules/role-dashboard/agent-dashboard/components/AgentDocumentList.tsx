...
import { FileText, Download, Trash2, CalendarIcon, Building, User } from "lucide-react";
...
export function AgentDocumentList({ propertyId, onDeleteDocument }: AgentDocumentListProps) {
  const documents = useAppSelector(propertyId 
              ...
              <div className="flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <div className="flex items-center gap-1 text-xs text-gray-600">
                        ...
                        <span>{document.tenantName || "No tenant assigned"}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs text-gray-600">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="truncate">{document.propertyAddress || "Property"}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <CalendarIcon className="w-3 h-3" />
                    <span>Valid until {format(document.validUntil, "MMM dd, yyyy")}</span>
                  </div>
                </div>
                
                <div className="text-xs text-gray-500 mt-2">
                  Uploaded on {format(document.uploadedDate, "MMM dd, yyyy 'at' h:mm a")}
                </div>
              </div>
