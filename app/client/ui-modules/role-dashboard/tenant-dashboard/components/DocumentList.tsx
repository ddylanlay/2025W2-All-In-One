...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "../../../theming-shadcn/Dialog";
    ...
    return (
      <div className="bg-white rounded-lg border p-6">
        <div className="text-center py-8">
          ...
          <p className="text-gray-500">Upload your first lease agreement document to get started.</p>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="bg-white rounded-lg border">
        <div className="px-6 py-4 border-b">
          ...
          <p className="text-sm text-gray-600">Manage your lease agreement documents</p>
        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        <div className="flex items-center gap-1">
                          ...
                          <span>Uploaded: {format(new Date(document.uploadedDate), 'MMM dd, yyyy')}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Calendar className="w-4 h-4" />
                          <span>Valid until: {format(new Date(document.validUntil), 'MMM dd, yyyy')}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {format(new Date(selectedDocument.uploadedDate), 'PPP')}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Valid Until</label>
                  <p className="text-sm text-gray-900">
                    {format(new Date(selectedDocument.validUntil), 'PPP')}
                  </p>
                </div>
              </div>
