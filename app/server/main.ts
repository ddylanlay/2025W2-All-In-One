...
import { InspectionCollection, ListingCollection, ListingStatusCollection } from "/app/server/database/property-listing/listing-collections";
...
import { LandlordCollection, ProfileCollection } from "./database/user/user-collections";
...
async function tempSeedUserAndRoleData(): Promise<void> {
  ...
  for (const user of seedUsers) {
    ...
    try {
      const { userAccountId } = await Meteor.callAsync(MeteorMethodIdentifier.USER_REGISTER, user);
...
async function seedPropertyCoordinatesForTempProperties(): Promise<void> {
  ...
  for (const coord of coordinates) {
    const existing = await PropertyCoordinatesCollection.findOneAsync({ _id: coord._id });
    if (!existing) {
...
async function seedListedProperties(agent: ApiAgent, landlord: ApiLandlord, tenant: ApiTenant): Promise<void> {
  ...
  async function seedProperties(): Promise<void> {
    ...
    const properties: Array<PropertyDocument> = [
      ...
      },
      {
        ...
        postcode: "3000",
        property_status_id: propertyStatusVacantId,
        description: "Modern 1-bedroom apartment in the heart of the city. Close to amenities and public transport.",
      ...
      },
      {
        ...
        postcode: "3220",
        property_status_id: propertyStatusVacantId,
        description: "Luxurious 5-bedroom house with stunning ocean views. Features a private pool and spacious garden.",
      ...
      },
      {
        ...
        postcode: "3350",
        property_status_id: propertyStatusVacantId,
        description: "Charming 3-bedroom cottage with a cozy fireplace and large backyard. Perfect for families.",
      ...
      },
      {
        ...
        postcode: "3030",
        property_status_id: propertyStatusVacantId,
        description: "Stylish 2-bedroom apartment with modern finishes and a balcony overlooking the river.",
      ...
      },
      {
        ...
        postcode: "3175",
        property_status_id: propertyStatusVacantId,
        description: "Spacious 4-bedroom family home with a large garden and double garage. Close to schools and parks.",
      ...
      },
      {
        ...
        postcode: "7000",
        property_status_id: propertyStatusVacantId,
        description: "Elegant 3-bedroom townhouse with modern amenities and a private courtyard. Ideal for professionals.",
    ...
    for (const property of properties) {
      ...
      if (existingProperty) {
        console.log(`[Seed] Skipped inserting existing property in PropertyCollection: ${property._id}`);
        continue;
      }
