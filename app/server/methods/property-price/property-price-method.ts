...
const insertPropertyPriceForProperty = { [MeteorMethodIdentifier.PROPERTY_PRICE_INSERT]: async (
propertyPriceData: PropertyPriceInsertData
): Promise<string> => {
    const propertyPrice = await PropertyPriceCollection.insertAsync(propertyPriceData);
